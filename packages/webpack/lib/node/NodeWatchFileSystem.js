...
/**
 * Watchpack 是一个专门用于监听文件系统变化的库，主要由 Webpack 团队开发和维护
 * 用于监听文件和目录的变化，并通过事件的形式通知应用程序
 * Watchpack 是 Webpack 的关键依赖之一，帮助其在开发模式下实时监听文件变化，以便自动重新编译或刷新页面
 * Webpack 使用它来监听项目中 JavaScript 文件、CSS 文件、配置文件等的变动
 *
 * - 监听文件与目录：可以监控文件和目录的增删、修改，支持批量监听
 * - 事件聚合：将短时间内的多次变动聚合成一个事件，以减少频繁触发带来的性能消耗
 * - 缓存时间戳：Watchpack 还会记录文件的时间戳信息，用于优化检测逻辑，从而减少不必要的重新编译
 */
